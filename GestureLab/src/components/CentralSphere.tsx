...
export interface CentralParams {
  ...
  sphereScale: number; haloRadius: number
  autoScale: boolean
  spinSpeed: number
  autoRotation: boolean
  glowOpacity: number; hue: number
  autoColor: boolean
}
...
export const defaultCentralParams: CentralParams = {
  ...
  autoPosition: true,
  sphereScale: 1, haloRadius: 1.15,
  autoScale: true,
  spinSpeed: 0.3,
  autoRotation: true,
  glowOpacity: 0.18, hue: 0.5,
  autoColor: true,
}

const Svg = ({ d, viewBox = "0 0 16 16" }: { d: string; viewBox?: string }) => (
  <svg width="14" height="14" viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <path d={d} />
  </svg>
)
...
export const MODE_INFO: Record<number, { label: string; icon: ReactNode }> = {
  ...
  1: { label: "Move", icon: <Svg d="M8 2v12M2 8h12M5 5l3-3 3 3M5 11l3 3 3-3" /> },
  2: { label: "Scale", icon: <Svg d="M2 14L14 2M6 14h8V6" /> },
  3: { label: "Color", icon: <svg width="14" height="14" viewBox="0 0 16 16"><circle cx="8" cy="8" r="7" fill="currentColor" /></svg> },
  4: { label: "Rotate", icon: <Svg d="M14 8A6 6 0 1 1 8 2c1.9 0 3.6.9 4.7 2.3M14 2v4h-4" /> },
}
...
export function CentralSphere({
  ...
  useFrame(({ clock, size, camera, pointer }) => {
    ...
    landmarks.forEach((hand) => {
      ...
      for (let i = 0; i < 21; i++) {
        ...
        const t = new THREE.Vector3(
          (lm.x - 0.5) * wX, (0.5 - lm.y) * wY, (lm.z ?? 0) * 0.35,
        )
    ...
    if (prevMode.current === 1 && curMode !== 1) {
      ...
      onParamsChange({ posX: gPos.current.x, posY: gPos.current.y, posZ: gPos.current.z, autoPosition: false })
    }

    /* ===== Position ===== */
    let tx = p.posX, ty = p.posY, tz = p.posZ
    ...
    sPos.current.lerp(new THREE.Vector3(tx, ty, tz), draggingRef.current ? 1 : lerpFactor)
    ...
    if (curMode === 4 && hasOther && hasManip) {
      ...
      if (curMode === 0 && p.autoRotation && !hasCustomRot.current) {
        sRotY.current += (Math.sin(clock.elapsedTime * 0.55) * 0.18 - sRotY.current) * 0.05
      } else {
    ...
    pm.opacity = 0.07 + intensity * 0.14 + prox * 0.22
    pm.color.setHSL(sHue.current + 0.02, sSat.current + 0.08, 0.5)
  ...
  const onDown = (e: ThreeEvent<PointerEvent>) => {
    ...
    dragOffRef.current.set(e.point.x - sPos.current.x, e.point.y - sPos.current.y, 0)
